    ...
    try {
      ...
      for (const product of productsData) {
        if (product.hasVariations) {
          ...
          try {
            const varResponse = await axios.get(`${API_URL}/products/${product._id}`);
            
            // Check if response has the expected structure
            if (varResponse.data && varResponse.data.data) {
              const variations = varResponse.data.data.variations || [];
            ...
            console.error(`Error fetching variations for product ${product._id}:`, error.response?.data || error.message || error);
          }
        } else {
          // Add product as is
          expandedProducts.push({
